...
const WorkoutHistoryTab: React.FC = () => {
  const [history, setHistory] = useState<CompletedSession[]>(MOCK_HISTORY);
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        if (res.sessions && res.sessions.length > 0 && isMounted) {
          const formatted: CompletedSession[] = res.sessions.map((s: any) => ({
            id: s.id,
            date: s.completedAt ? new Date(s.completedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'Completed',
        ...
        console.log('Failed to fetch history from API, showing default mock history');
      } finally {
